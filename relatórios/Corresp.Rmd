...
dt <- sdata[sdata$ocorr_solic %in% os_dt,] %>% group_by(year(data_solic), date(data_solic), ocorr_solic) %>% 
                                          ...
                                          filter(n_solic >= 100, `year(data_solic)` >= 2005, ocorr_solic == "DESLIZAMENTO DE TERRA") 
...
dos_os %>% filter(desc_orig_solic == "199") %>% 
           ...
           coord_cartesian(ylim = c(0, 100)) +   
                                         ...
                                         theme(legend.position = "bottom", legend.box = "horizontal")+
                                          labs(title = "Indicador de verdadeiro-positivo (C) para solicitações abertas via 199.",
                                               subtitle = "",
                                              caption = "Período analisado: 07/05/2020 a 31/12/2019", 
                                              x = "Ano",
                                              y = "C") 
...
Podemos observar no gráfico acima que o valor de C após 2015 matém o mesmo comportamento que nos anos anteriores, oscilando entre 50% e 65%.
