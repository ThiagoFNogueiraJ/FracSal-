...
sdata[sdata$ocorr_solic %in% os_dt,] %>% group_by(year(data_solic), ocorr_solic, desc_orig_solic) %>% 
                                         summarise(n = mean(match)*100) %>% 
                                         ggplot() + geom_line(aes(x = `year(data_solic)`, y = n, colour = as.factor(ocorr_solic))) + facet_wrap(~desc_orig_solic)
