...
#### Qauntidade de solicitações e data de abertura - Teste de normalidade
...
OS gráficos de boxplot, histograma e qq
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#Boxplot
...
nteste <- data.frame(n_sol = seq(1, max(dt_nteste$n_solic),1),
                     n_C = 0,
                     p_value =  0,
                     media = 0,
                     dp = 0)
 ...
 for (i  in n_sol) {
  ...
  nteste[nteste$n_sol == i,]$p_value <- ifelse(nteste[nteste$n_sol == i,]$n_C >= 3, shapiro.test(z)$p.value, NULL)
...
dt_nteste %>% ggplot() + geom_boxplot(aes(x=C)) +coord_flip() + xlim(0,1) +
                        transition_reveal(n_solic)
